...
**Re-verified 2026-07-08 (reorg pass):** `docs/add-component-loop-case-study.html` is
untouched since 2026-06-23 and still narrates the old single-command flow (references
"Stage 3"/"Stage 4" inside one continuous `/add-component` run, no mention of
`/review-component` as a separate delegate or the Stage 2b visual checkpoint) — the
refresh below has not happened. Status stays `active`.
