...
async function withStory(id, theme, fn) {
...
async function shot(id, { theme, out }) {
  const file = out
    ? (isAbsolute(out) ? out : join(process.cwd(), out))
    : join(HERE, 'shots', `${id}${theme ? '.' + theme : ''}.png`)
  mkdirSync(dirname(file), { recursive: true })
  const errors = await withStory(id, theme, async (page, errs) => {
...
async function errorsCmd(id, { theme }) {
  const errors = await withStory(id, theme, async (_page, errs) => errs)
