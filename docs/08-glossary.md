...
**Screenshot baseline**
A committed PNG file capturing the expected visual output of a component's canonical story in one theme (light or dark). Unlike `visualReview` (a human judgment), a screenshot baseline is a reference image used for automated perceptual diff (ADR-019) — running `npm run screenshot:check` compares fresh screenshots against baselines to catch unintended pixel shifts. Stored in `packages/components/screenshots/`, named `<story-id>.light.png` / `<story-id>.dark.png`; re-baselined with `npm run screenshot:approve` after an approved visual change. Not to be confused with frozen-snapshot "baseline" (the `airtable-governance.json`, `token-usage.json` sense).
...
**Component metadata**
The machine-readable fact sheet each component ships alongside its code — a `<Name>.metadata.json` file listing its purpose, variants, states, design tokens, accessibility contract, and where it fits among other components (what it accepts as children, what can contain it). It's validated in CI against a schema, so it can't merge in a malformed or incomplete shape, and every token path it references is checked against the real token source rather than just the file's own formatting. Agents read this file instead of a component's implementation code when deciding how to use, scaffold, or generate around it — it's what makes their output checkable against a real contract rather than an inferred guess. See [Machine-readable metadata](10-machine-readable-metadata.md). See also Deterministic gate, Frozen snapshot, Pattern aggregate.
...
**Pattern aggregate**
`.claude/component-patterns.json` — a generated file that scans every component's metadata and JSX and groups them by shared interaction pattern (disclosure, navigation, form field, and seven others), so the system has one place that shows which components work alike instead of only how each one works on its own. It also carries a `drift` list flagging where a component's actual code disagrees with what its own metadata claims — a prop named differently than expected, a composed-of list that's gone stale. Regenerated by `npm run patterns:generate` and read only by `/layout-generation`, never by component scaffolding — a measured comparison found it helped one and hurt the other. See [Machine-readable metadata](10-machine-readable-metadata.md). See also Component metadata, Frozen snapshot.
...
**Clock reset**
A frontmatter comment on a `docs/NN-*.md` page that tells `docs:check` "I looked at what changed in a listed source, and this page is still accurate" — without rewriting any prose. Every doc page declares its load-bearing `sources:` in frontmatter, and `npm run docs:check` fails when a source has a newer commit than the page, flagging it stale. Sometimes the source really did change but nothing the page says became wrong; appending a line like `# clock reset 2026-07-23: adds docs-scribe rubric summary...; see docs/11-self-improving-loops.md` and committing it resets the staleness clock honestly, because that comment's own commit becomes the page's new "last touched" time. Contrast this with a real `/docs-sync` rewrite, where the prose itself changes because the page actually went stale — a clock reset is for the narrower case where a source moved but the description of it didn't need to.
...
**Ledger**
An append-only record of events, kept specifically so nothing already recorded is ever edited or deleted — only added to. `.claude/handoff/run-ledger.json` is a ledger: every `/review-component` run's gate result and reviewer findings get appended to it, deduped by component and timestamp, so the history of runs survives even though the per-run JSON files themselves are gitignored. Its most load-bearing field is `reviewerFindingsBeyondGateCount` — how many findings the adversarial reviewer caught that the deterministic gate could not — kept specifically so the question "is the adversarial-review stage worth its usage-window cost" can be answered empirically from accumulated runs instead of argued from intuition. See [Self-improving loops](11-self-improving-loops.md).
