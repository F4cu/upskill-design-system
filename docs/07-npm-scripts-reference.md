...
`metadata:validate`, `typecheck`, `build`, `a11y:coverage`, and `a11y:test` run automatically in `components-check.yml` on every PR. Run them locally before pushing a new component. `screenshot:check` runs advisory on the same gate at a loosened 1% ratio; promote to blocking only once CI diff ratios prove quiet against ubuntu antialiasing drift (see ADR-019 escalation path).
