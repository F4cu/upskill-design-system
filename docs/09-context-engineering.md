...
| 2 | `.claude/rules/*.md` | Only when the session touches a file matching the rule's `paths:` globs | Package-scoped conventions — `components.md` (`packages/components/**`), `tokens.md` (`packages/tokens/**`). |
...
One naming trap, stated once: Claude Code's terms are the inverse of intuition. `.claude/commands/` holds prompt-only slash commands (all nine agentic moments); `.claude/skills/` holds commands that ship companion *code* — here only `run-storybook`, whose `driver.mjs` drives Storybook headlessly. Since the 2026-07-08 audit, every command file also carries `model:` and `allowed-tools:` frontmatter, turning "this moment must not use MCP" from a remembered convention into a tool boundary (see [Agentic moments](06-agentic-moments.md)).
