...
## 1. The parallel agent swarm (ADR-007)

**What was proposed:** a blueprint called "In-Demand, API-Driven Agentic Loop Architecture" — a hub-and-spoke router spawning 2–3 parallel worker agents per task, an asymmetric Sonnet-router/Haiku-worker split billed per token, Figma variables pulled live over REST, and an adversarial verifier gating the output.

**Why it was rejected:** three of its four load-bearing assumptions don't hold on this system's actual runtime. It's built for Claude Pro, where the constraint is a rolling weekly usage window, not per-token dollars — so parallel workers, which the blueprint assumes are "pennies" cheap, are in fact the *fastest* way to exhaust the one resource that's genuinely scarce. Running three workers at once doesn't cost three times the money; it drains the same shared window three times as fast and hits a rate limit sooner. The REST-based Figma pull failed for an unrelated, simpler reason: the Variables REST API is Enterprise/Org-only, and this isn't an Enterprise plan — the endpoint doesn't exist to poll. The per-token Haiku-worker cost tiering assumed API credits that don't exist on a Pro seat at all.

**What survived:** the three ideas that weren't plan-dependent — frozen state-file handoffs, one adversarial verification stage, and on-demand triggering instead of anything continuous. The loop that shipped (ADR-007's Option 2: sequential, at most two agents, frozen-file handoffs) is what's left after stripping the assumptions that don't transfer.

**The honest framing:** this isn't "we didn't need parallelism." It's "the blueprint's parallelism was optimized for a billing model we don't have, and porting it anyway would have been slower and hit rate limits faster than doing nothing." A different Anthropic plan (API credits, Enterprise Figma) would make a different call here — the rejection is plan-specific, not a universal claim that parallel agents are bad.

## 2. Figma as the primitives source of truth (ADR-002, reversed)

**What was originally decided:** designers author color/spacing/typography variables in Figma; an automated sync pulls them into `primitives.json`; the JSON file is never hand-edited.

**Why it was reversed:** the same Enterprise wall shows up here from a different angle. The Variables REST API is Enterprise-only (no automated Figma→code pull is possible at all), Code Connect is equally gated, and Token Studio's free-tier GitHub sync only handles a single file while this architecture is deliberately split across six-plus files (primitives, two brand files, two theme files, three device files). No available tool can do the sync this design needs. The only direction that *is* automatable — code into Figma, interactively via the plugin — is the reverse of what was planned.

**What shipped instead:** primitives.json became hand-edited via PR like every other layer, Figma became a downstream mirror and design-exploration surface, and a value invented in Figma is treated as a proposal, not a fact, until it lands in the committed JSON via `/figma-variable-audit`'s drift check.

**The honest framing:** this is arguably the most consequential reversal in the system, because it inverts the intuitive design-tool-owns-design-decisions model. It's also the cleanest example of "the plan changed the architecture, not the other way around" — nobody decided code-as-source-of-truth was philosophically superior; the tooling made the alternative impossible to automate, and code-as-source-of-truth was what remained.

## 3. `$root` as the group-default token suffix (ADR-003, superseded)
...
## 4. Feeding cross-component pattern context into every generation task
...
**Why it was rejected — with data, not intuition:** ADR-013 ran a before/after harness, 7 pre-registered tasks across scaffold, composition, and layout task kinds, each scored twice — once with per-component metadata only, once with the pattern file added — against the repo's real deterministic gates plus a trap checklist. The result didn't support the "more context always helps" intuition: the pattern file measurably *improved* composition and layout tasks (violations roughly halved) and measurably *worsened* component scaffolds (gate violations rose from 11 to 17). The likely mechanism, stated in the ADR rather than left implicit: the ~23K-character aggregate crowds out attention on the specific metadata-schema and file-contract gates a scaffold task needs to hit.
...
**The honest framing:** this is the smallest-scope rejection in the list, and unlike the other four it isn't a plan constraint (billing tier, API gating, ecosystem convention) forcing the call — it's a case of catching a self-inflicted design smell before it became load-bearing. The value of a fixed component set with a small enum surface is that styling is supposed to be a closed decision, made once in the design system, not a per-instance choice; a pattern that technically enforced the enum's *values* while leaving the *mechanism* open-ended undercut that guarantee quietly. The fix cost nothing at runtime (verified with a full screenshot-diff pass, 54/54 shots at 0.000% diff) — it's a pure case of the DOM shape catching up to what the type system already claimed.

## The pattern across all five

Look at the five rejections together and a shape emerges: none of them were rejected because the alternative was a bad idea in the abstract. A parallel swarm is a fine architecture — on API billing. Figma-owned primitives is a fine architecture — on an Enterprise plan with working REST sync. `$root` is a fine-sounding convention — if the ecosystem hadn't already converged elsewhere. Feeding more context into every task is a fine intuition — until measured against the specific task it's fed into. Custom-property-via-`style` is a fine technique — until you notice the attribute it relies on is the same one that lets anyone write arbitrary CSS. Each rejection is **conditional on a fact about this system's actual constraints** (Pro-tier billing, non-Enterprise Figma, ecosystem convention, measured task sensitivity, DOM-level enforcement guarantees), not a claim that the rejected approach is wrong everywhere. That conditionality is what makes the rejections trustworthy rather than self-serving: they're falsifiable, and in one case, already tested against real data.
