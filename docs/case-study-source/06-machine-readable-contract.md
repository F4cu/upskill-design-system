...
I didn't pick JSON over Markdown for aesthetic reasons — I picked it because someone else had already measured the difference and published the numbers. Research benchmarking multiple context configurations across a thousand-plus prompts found that structured JSON cut token cost by roughly 80% against equivalent Markdown, at equal or better accuracy, for exactly this kind of "give an agent facts about a component" task. The reasoning tracks with what I'd expect from working with these systems day to day: JSON is unambiguous — explicit keys, explicit values, nothing to parse out of prose — while Markdown makes an agent do interpretive work on every read that a schema does once, at write time.
