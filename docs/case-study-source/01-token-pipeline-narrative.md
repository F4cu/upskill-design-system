...
ADR-002 first declared primitives Figma-owned: designers author variables in Figma, an automated sync pulls them into code, primitives.json is "never hand-edited." That's the conventional shape, and it's the one this system tried first.

It broke on plan limits, not on principle. Three things are true simultaneously on a non-Enterprise Figma plan:

- The **Variables REST API is Enterprise/Org-only** — there is no way to poll Figma variables and pull them into a script or CI job.
- **Code Connect is also Enterprise/Org-only** — the other conventional bridge is gated the same way.
- **Token Studio's free-tier GitHub sync is single-file**, and this architecture is deliberately six-plus files (primitives, two brand files, two theme files, three device files) — the free-tier tool can't represent the shape the tokens actually need.

Put together: there is no automatable path from Figma into code. The only thing that *can* be automated is the reverse — code into Figma, and even that only interactively, through the plugin (MCP), never a scheduled job. Once that's the ceiling, "Figma owns primitives" isn't a design choice anymore, it's a plan someone can't execute. ADR-002's 2026-06-17 amendment records the reversal plainly: primitives.json becomes hand-editable via PR like every other layer, and **a value invented in Figma is a proposal, not a fact, until it lands in the committed JSON.**
